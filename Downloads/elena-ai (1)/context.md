...
│   │   ├── SupabaseBackupPanel.tsx# Panel de respaldos en la nube
...
* **[2026-08-23] Personalización de Tickets Térmicos (58mm / 80mm / Comunes) y Control de Visibilidad por Departamento:**
  * **Personalización de Comprobante Térmico de Pedidos:** Integrado el componente `ThermalPedidoModal.tsx` y la descarga PDF adaptativa con las configuraciones de cabecera de "Impresión Térmica".
  * **Desglose Financiero en Dólares (USD $):** Formato limpio mostrando únicamente Total Presupuestado, Anticipo Recibido y Saldo Restante en USD ($).
  * **Control de Acceso y Visibilidad por Departamento:** Añadido campo `departamento` a `Usuario` y filtrado automático de trabajos en `Pedidos.tsx`.
